...
{
    // В данный индикатор добавил линию со значением = 0.
    // Мне нравится этот индикатор.
    [Indicator("ForceIndexZero")]
    public class ForceIndexZero : Aindicator
    {
		...
		public IndicatorDataSeries ZeroValues;
		...
		public override void OnStateChange(IndicatorState state)
        {
            if (state == IndicatorState.Configure)
            {
                ...
                _period = CreateParameterInt("Period", 43);
                _candlePoint = CreateParameterStringCollection("Candle point", "Close", Entity.CandlePointsArray);
				ZeroValues = CreateSeries("ZeroLine", color: System.Drawing.Color.AliceBlue, IndicatorChartPaintType.Line, true);
        ...
        {
            // Сделать отображение значения 0
            decimal zero = 0.001m;
            ZeroValues.Values[index] = zero;
